...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Kfold run')
    parser.add_argument('-m', '--model', help='modelname', required=True)
    ...
    # ----------Data-------------
    train_file = ('../200-window_split_train_p1.csv')
    ...
    overall_training_status = []
    kfold_dir = './kfold_' + model_name.replace('/', '_') + '/'
    os.makedirs(kfold_dir, exist_ok=True)
    for kfold in range(10):
        print('KFOLD: {}/10'.format(kfold + 1))
        ...
        overall_training_status.append(training_status)
        ...
        accuracy = accuracy_score(test_labels, result_label)
        precision = precision_score(test_labels, result_label, average="macro")
        recall = recall_score(test_labels, result_label, average='macro')
        f1 = f1_score(test_labels, result_label, average="macro")
        report = classification_report(true_labels_array, result_label, target_names=target_names)
        ...
        with open(output_dir + 'results.txt', 'w') as outfile:
        ...
        cross_val_results["Accuracy"].append(accuracy)
        cross_val_results["Precision"].append(precision)
        cross_val_results["Recall"].append(recall)
        cross_val_results["F1"].append(f1)
    ...
    kfold_accuracy = sum(cross_val_results["Accuracy"]) / len(cross_val_results["Accuracy"])
    kold_precision = sum(cross_val_results["Precision"]) / len(cross_val_results["Precision"])
    kfold_recall = sum(cross_val_results["Recall"]) / len(cross_val_results["Recall"])
    kfold_f1 = sum(cross_val_results["F1"]) / len(cross_val_results["F1"])

    print("10-fold Accuracy     ", kfold_accuracy)
    print("10-fold Precision    ", kold_precision)
    print("10-fold Recall       ", kfold_recall)
    print("10-fold F1           ", kfold_f1)

    with open(kfold_dir + 'results.txt', 'w') as outfile:
        outfile.write("Accuracy     " + str(kfold_accuracy) + "\n")
        outfile.write("Precision    " + str(kold_precision) + "\n")
        outfile.write("Recall       " + str(kfold_recall) + "\n")
        outfile.write("F1           " + str(kfold_f1) + "\n")

    plt.figure()
    plt.plot(range(10), cross_val_results["Accuracy"], label='Accuracy')
    plt.title('Stats ({})'.format(model_name))
    plt.xlabel('10-fold')
    plt.plot(range(10), cross_val_results["Precision"], label='Precision')
    plt.plot(range(10), cross_val_results["Recall"], label='Recall')
    plt.plot(range(10), cross_val_results["F1"], label='F1')
    plt.legend()
    plt.show()
    plt.savefig(kfold_dir + 'Stats.png')
