...
if __name__ == '__main__':
    ...
    # ----------Data-------------
    test_file = ('../200-window_split_200_release2.csv')
    ...
    _, categories_df = numeric_category(test_category)
    ...
    model_path = './kfold_bert-large-uncased/20220430-014744/'
    ...
    df = pd.DataFrame(list(zip(
         test_data['text'].to_list(), classification, test_data['word'].to_list(),
         test_data['start'].to_list(), test_data['end'].to_list(), test_data['type'].to_list(),
         test_data['file'].to_list())),
        columns=['sentence', 'pred_label', 'drug', 'span_start', 'span_end', 'anno_type', 'filename'])

    df.to_csv('prediction_results_200_release2_bert_large.csv')
